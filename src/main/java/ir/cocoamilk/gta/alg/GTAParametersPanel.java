...
public class GTAParametersPanel extends JPanel
		implements ColumnCreatedListener, CytoPanelComponentSelectedListener {
	...
	private java.awt.Choice subnetMaxSize;
...
//		aboutButton.setPreferredSize(new java.awt.Dimension(67, 23));
	...
	private static GridBagConstraints gridConstraint(int gridx, int gridy,
...
//		TableColumn nameColumn = normalAttributeSelectorTable.getColumn(AttrSelectionTableModel.COL_NAME);
//		nameColumn.setCellRenderer(new NameColumnCellRenderer());
//		cancerAttributeSelectorTable.getColumn(AttrSelectionTableModel.COL_NAME).setCellRenderer(new NameColumnCellRenderer());
		

//		// give user the option to switch sig
//		TableColumn switchSigColumn = normalAttributeSelectorTable.getColumn(AttrSelectionTableModel.COL_REVERSE_SIG);
...
//		
//
//		CheckBoxCellEditorListener checkBoxCellEditorListener = new CheckBoxCellEditorListener(normalAttributeSelectorTable.getModel());
...
//		TableColumn normColumn = normalAttributeSelectorTable.getColumn(AttrSelectionTableModel.COL_SCALING);
...
//		
//		cancerAttributeSelectorTable.getColumn(AttrSelectionTableModel.COL_SCALING).setCellRenderer(new NormalizationCellRenderer());
//		cancerAttributeSelectorTable.getColumn(AttrSelectionTableModel.COL_SCALING).setCellEditor(new NormalizationComboboxEditor());
		...
		configPanel.add(bothAttributeSelectorPanel, gridConstraint(0, 1, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.BOTH));
		...
		advancedConfigPanel.add(generateTScoresCheckBox, gridConstraint(0, 0, 1, 2, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL));

		generateDegreesCheckBox = new Checkbox("Generate degrees");
		generateDegreesCheckBox.setState(true);
		advancedConfigPanel.add(generateDegreesCheckBox, gridConstraint(0, 1, 1, 2, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL));

		java.awt.Label subnetMaxSizeLabel = new java.awt.Label("Subnet max size");
		advancedConfigPanel.add(subnetMaxSizeLabel, gridConstraint(0, 2, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL));
		subnetMaxSize = new java.awt.Choice();
		for (int i=1; i<GTAAlgParameters.SUBNETWORK_MAX_SIZE_MAX; i++)
			subnetMaxSize.add(""+i);
		subnetMaxSize.select(""+GTAAlgParameters.SUBNETWORK_MAX_SIZE_DEFAULT);
		advancedConfigPanel.add(subnetMaxSize, gridConstraint(1, 2, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL));

		java.awt.Label nuumberOfTargetNetworksLabel = new java.awt.Label("No. of results");
		advancedConfigPanel.add(nuumberOfTargetNetworksLabel, gridConstraint(0, 3, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL));
		...
		advancedConfigPanel.add(numberOfTargetNetworks, gridConstraint(1, 3, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL));

		javax.swing.JScrollPane advancedConfilScroll = new javax.swing.JScrollPane(ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		advancedConfilScroll.setViewportView(advancedConfigPanel);

		configPanel.add(advancedConfilScroll, gridConstraint(0, 2, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.BOTH));
...
//		Dimension tableSize = normalAttributeSelectorTable.getPreferredSize();
//		normalAttributeSelectorPanel.setPreferredSize(new Dimension(normalAttributeSelectorPanel.getWidth(), (tableSize.height + 50)));
//		cancerAttributeSelectorPanel.setPreferredSize(new Dimension(cancerAttributeSelectorPanel.getWidth(), (tableSize.height + 50)));
//		
...
//		private String TOOLTIP_EXCLAIMATION = "<html>GTA requires that any numeric node attribute<br>"
...
//	}
	...
	private class AttrSelectionTableModel extends AbstractTableModel {
		...
		public final static int COL_NAME_ID = 0;//, COL_MOST_SIG_ID=1, COL_LEAST_SIG_ID=2, COL_REVERSE_SIG_ID=3, COL_SCALING_ID=4; 
		public final static String COL_NAME = "Name";//, COL_MOST_SIG="Most sig", COL_LEAST_SIG="Least sig", COL_REVERSE_SIG="Reverse sig", COL_SCALING="Scaling"; 
		private final String[] columnNames = { COL_NAME};//, COL_MOST_SIG, COL_LEAST_SIG, COL_REVERSE_SIG, COL_SCALING };
...
//						model.getValueAt(i, AttrSelectionTableModel.COL_REVERSE_SIG_ID).toString());
//				double min = Double.valueOf(
//						model.getValueAt(i, AttrSelectionTableModel.COL_MOST_SIG_ID).toString());
//				double max = Double.valueOf(
//						model.getValueAt(i, AttrSelectionTableModel.COL_LEAST_SIG_ID).toString());
...
//				if (!isPValue) {
//
//					row[AttrSelectionTableModel.COL_SCALING_ID] = isPValue ? ScalingMethodX.NONE.getDisplayString()
//							: ScalingMethodX.RANK_UPPER.getDisplayString();
//				} else
//					row[AttrSelectionTableModel.COL_SCALING_ID] = ScalingMethodX.NONE.getDisplayString();
//
//				dataVect.add(row);
//			}
//		}
...
//			if (!(Math.min((Double) row1[AttrSelectionTableModel.COL_MOST_SIG_ID], (Double) row1[AttrSelectionTableModel.COL_LEAST_SIG_ID]) >= 0
//					&& Math.max((Double) row1[AttrSelectionTableModel.COL_MOST_SIG_ID], (Double) row1[AttrSelectionTableModel.COL_LEAST_SIG_ID]) <= 1)) {
//				return 1;
//			}
//			return 0;
//		}
...
//	}
	...
	public class FindModulesAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			final AttrSelectionTableModel normalModel = (AttrSelectionTableModel) normalAttributeSelectorTable.getModel(),
					cancerModel = (AttrSelectionTableModel) cancerAttributeSelectorTable.getModel();

			GTAAlgParametersAttrSelection normalSelParam= new GTAAlgParametersAttrSelection(), cancerSelParam = new GTAAlgParametersAttrSelection();
			...
			for (int j=0; j<selParams.length; j++) {
				for (int i=0; i<selectedIndices[j].length; i++) {
					selParams[j].getSelectedNames().add((String) models[j].getRow(selectedIndices[j][i])[AttrSelectionTableModel.COL_NAME_ID]);
//					selParams[j].getSwitchSigs().add((Boolean) models[j].getRow(selectedIndices[j][i])[AttrSelectionTableModel.COL_REVERSE_SIG_ID]);
//					selParams[j].getScalingMethods().add((String) models[j].getRow(selectedIndices[j][i])[AttrSelectionTableModel.COL_SCALING_ID]);
				}
			}
...
//			pluginMainClass.startFindActivePaths(networkPanel.getSelectedNetwork());
		...
		private void runAlgorithm(GTAAlgParametersAttrSelection normalSelParam,
			...
			GTAAlgParameters params = new GTAAlgParameters(networkSelectorPanel.getSelectedNetwork(), normalSelParam, cancerSelParam, 
					generateTScoresCheckBox.getState(), generateDegreesCheckBox.getState(), Integer.parseInt(subnetMaxSize.getSelectedItem()),
					Integer.parseInt(numberOfTargetNetworks.getSelectedItem()));
